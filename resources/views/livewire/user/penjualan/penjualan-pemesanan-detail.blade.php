<div class="overflow-x-hiden">
    ...
    <div class="flex items-center space-x-2 mb-5">
        ...
        <span class="text-gray-400 text-sm">/</span>
        <a href="{{ route('user.penjualan.pemesanan') }}" class="text-gray-400 hover:text-primary-500">Pemesanan</a>
        <span class="text-gray-400 text-sm">/</span>
    ...
    <div class="bg-gray-50 rounded p-8 soft-scrollbar mb-12 dark:bg-gray-800 transition duration-200 ">
        <div class="flex content-center mb-10">
            <div class="flex flex-none w-1/3">
                <h4 class="text-2xl font-semibold dark:text-gray-100">Detil Pemesanan - {{ $pemesanan->kode }}</h4>
            </div>
            <div class="flex flex-initial w-2/3 justify-end">

                <x-button warning label="Kembali" icon="chevron-left" href="{{ route('user.penjualan.pemesanan') }}" />
        ...
        <div class="border border-grey-200 grid grid-cols-1 gap-2 rounded">
            <div class="mt-0 border-b border-grey-200 p-6 grid grid-cols-2 gap-4">
                ...
                <div class="flex flex-row-reverse">
                    ...
                    <div class="mx-2">
                        <x-button red label="Batalkan Pemesanan" icon="x" data-popover-target="popover-batalkan"
                            data-popover-trigger="click" href="" />
                    </div>
                    <div data-popover id="popover-batalkan" role="tooltip"
                        ...
                        <div
                            ...
                            <h3 class="font-semibold text-gray-900 dark:text-white">Anda yakin mebatalkan pemesanan ini
                            </h3>
                        </div>
                        <div class="px-3 py-2">
                            <x-button blue label="OK" wire:click.prevent="batalPesanan" />
                        </div>
                        <div data-popper-arrow></div>
                    </div>
                    <div class="ml-2">
                        <x-button blue label="Buat Tagihan" icon="plus"
                            href="{{ route('user.penjualan.tagihan.tambah.buat', ['tipe' => 'pemesanan', 'buat_id' => $pemesanan->id]) }}" />
                    </div>
                    <div class="ml-2">
                        <x-button green label="Buat Pengiriman" icon="plus"
                            href="{{ route('user.penjualan.pengiriman.tambah', ['pemesanan_id' => $pemesanan_id]) }}" />
                    </div>
            ...
            <div class="mt-0 px-6 pt-6 grid grid-cols-3 gap-2">
                <div class="flex flex-col">
                    <span class="text-base dark:text-white">Pelanggan</span>
                    <a href="{{ route('user.kontak.detail', ['kontak_id' => $pemesanan->kontak_id]) }}"
                        class="font-semibold text-base text-primary-500 dark:text-primary-300 hover:underline hover:text-primary-700 dark:hover:text-primary-500">{{ $pemesanan->kontak->name }}</a>
            ...
            <div class="mt-0 px-6 grid grid-cols-3 mb-6">
                <div class="flex flex-col">
                    ...
                    <span
                        class="font-semibold dark:text-white">{{ date('d/m/Y', strtotime($pemesanan->tanggal_transaksi)) }}</span>
                </div>
                <div class="flex flex-col">
                    <span class="font-medium text-sm mt-4 dark:text-white">Tanggal Jatuh Tempo</span>
                    <span
                        class="font-semibold dark:text-white">{{ date('d/m/Y', strtotime($pemesanan->tanggal_jatuh_tempo)) }}</span>
                </div>
            </div>
                    ...
                    <table class="w-full whitespace-nowrap mb-5">
                        <tbody>
                            <tr
                                ...
                                <td class="p-2 w-1/3">@currency($pemesanan->sub_total)</td>
                            </tr>
                            <tr
                                class="border-b border-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 text-right text-sm font-medium text-gray-900 dark:text-white">
                                <td class="p-2 w-2/4">Pajak</td>
                                <td class="p-2 w-2/4">@currency($pemesanan->pajak)</td>
                            </tr>
                            <tr
                                class="border-b border-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 text-right text-sm font-medium text-gray-900 dark:text-white">
                                <td class="p-2 w-2/4">Total</td>
                                <td class="p-2 w-2/4">@currency($pemesanan->total)</td>
                            </tr>
