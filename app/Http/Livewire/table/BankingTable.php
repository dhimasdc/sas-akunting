...
final class BankingTable extends PowerGridComponent
{
    ...
    */
    ...
    {
        return Akun::query()->where('kategori', 'Banking')->orWhere('kategori', 'Kredit');
    }
    ...
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ...
            ->addColumn('name', function (Akun $model) {
                return '<a href="' . route('user.akun', ['akun_id' => $model->id]) . '" class="font-medium text-blue-500 hover:underline">' . ($model->name) . '</a>';
            })
            ...
            ->addColumn('kategori')
            ->addColumn('sub_akun')
            ->addColumn('saldo', function (Akun $model) {
                return number_format(($model->saldo), 0, ',', '.');
            })
