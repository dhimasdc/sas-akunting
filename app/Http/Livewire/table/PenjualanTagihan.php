...
final class PenjualanTagihan extends PowerGridComponent
{
    use ActionButton;
    ...
    */
     ...
     *
     * @return Builder<\App\Models\Penjualan_Tagihan>
     */
    public function datasource(): Builder
    {
        return Penjualan_Tagihan::query()->with('kontak');
    }
    ...
    */
    public function addColumns(): PowerGridEloquent
    {
        return PowerGrid::eloquent()
            ->addColumn('id')
            ->addColumn('kode', function (Penjualan_Tagihan $model) {
                return '<a href="' . route('user.penjualan.tagihan.detail', ['tagihan_id' => $model->id]) . '" class="bg-primary-500 p-1 rounded text-white hover:underline">' . ($model->kode) . '</a>';
            })
            ->addColumn('kontak', function (Penjualan_Tagihan $model) {
            ...
            ->addColumn('tanggal_transaksi_formatted', fn (Penjualan_Tagihan $model) => Carbon::parse($model->tanggal_transaksi)->format('d/m/Y'))
            ->addColumn('tanggal_jatuh_tempo_formatted', fn (Penjualan_Tagihan $model) => Carbon::parse($model->kadaluarsa)->format('d/m/Y'))
            ->addColumn('status', function (Penjualan_Tagihan $model) {
            ...
            ->addColumn('sisa_tagihan', function (Penjualan_Tagihan $model) {
                return number_format(($model->sisa_tagihan), 0, ',', '.');
            })
            ->addColumn('total', function (Penjualan_Tagihan $model) {
    ...
    */
    ...
    /**
     * PowerGrid Penjualan_Tagihan Action Buttons.
     *
    ...
    public function actions(): array
    {
       return [
           Button::make('edit', 'Edit')
               ...
               ->route('penjualan_-tagihan.edit', ['penjualan_-tagihan' => 'id']),

           Button::make('destroy', 'Delete')
               ->class('bg-red-500 cursor-pointer text-white px-3 py-2 m-1 rounded text-sm')
               ->route('penjualan_-tagihan.destroy', ['penjualan_-tagihan' => 'id'])
               ->method('delete')
        ];
    }
    */
    ...
    */
    ...
    /**
     * PowerGrid Penjualan_Tagihan Action Rules.
     *
    ...
    public function actionRules(): array
    {
       return [
           ...
           //Hide button edit for ID 1
            Rule::button('edit')
                ->when(fn($penjualan_-tagihan) => $penjualan_-tagihan->id === 1)
                ->hide(),
        ];
    }
    */
