...
class PembelianTambahTagihanBuat extends Component
{
    use WithFileUploads;
    ...
    public function mount($tipe, $buat_id)
    {
        ...
        if ($this->tipe == 'penawaran') {
            $penawaran = Pembelian_Penawaran::find($buat_id);
            $this->vendor = $penawaran->kontak_id;
            ...
            $orderItems = ItemPembelianPenawaran::where('pembelian_id', '=', $penawaran->id)->get();
        ...
        if ($this->tipe == 'pemesanan') {
            $penawaran = Pembelian_Pesanan::find($buat_id);
            $this->vendor = $penawaran->kontak_id;
            $this->tgl_transaksi = $penawaran->tanggal_transaksi;
            $this->tgl_tempo = $penawaran->tanggal_jatuh_tempo;
            $this->termin = $penawaran->termin;
            ...
            $orderItems = ItemPembelianPemesanan::where('pembelian_id', '=', $penawaran->id)->get();
            foreach ($orderItems as $key => $items) {
                $this->orderProducts[$key]['produk_id'] = $items->produk_id;
                $this->orderProducts[$key]['harga'] = $items->harga;
                $this->orderProducts[$key]['deskripsi'] = $items->deskripsi;
        ...
        if ($this->tipe == 'pengiriman') {
            $penawaran = Pengiriman::with('pemesanan')->where('id', '=', $buat_id);
            $this->vendor = $penawaran->pemesanan->kontak_id;
            ...
            $orderItems = ItemPembelianPengiriman::where('pembelian_id', '=', $penawaran->id)->get();
            foreach ($orderItems as $key => $items) {
                $this->orderProducts[$key]['produk_id'] = $items->produk_id;
                $this->orderProducts[$key]['harga'] = $items->harga;
                $this->orderProducts[$key]['deskripsi'] = $items->deskripsi;
    ...
    public function addTagihan()
    {
        ...
        $transaksi->pembelian_tagihan_id = $tagihan->id;
        ...
        foreach ($this->orderProducts as $key => $items) {
            $items_p = new ItemPembelianTagihan();
        ...
        $this->notification()->success('Tagihan Pembelian Ditambahkan');
        redirect()->route('user.pembelian.tagihan.detail', ['tagihan_id' => $tagihan->id]);
    }
    ...
    public function render()
    {
        ...
        return view(
            'livewire.user.pembelian.pembelian-tambah-tagihan-buat',
